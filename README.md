...
| [`consistency`](docs/checks/consistency.md) | 複数ファイルから抽出した集合、または glob に一致するファイルパスの集合が一致するかの検証（type 一覧の突き合わせ、ドキュメントのページ集合と目次の突き合わせなど） |
...
理由の記載は必須です。空の免除は拒否されます。トレーラはメッセージ本文の最後の段落
（トレーラ段落）に書く必要があり、`doc-sync` のように検査によっては
`Doc-Sync: skip[docs/foo.md] 理由` の形で免除の対象を絞ることもできます。ローカルの
フックと CI が同じコミットメッセージを見るため、**手元で通した判断がそのまま CI でも
通ります**（環境変数による免除は用意していません。CI に届かず、手元では通ったのに
CI だけ落ちる、という状態になるためです）。
...
の 3 つのトップレベルキーを持ちます。`checks` に検査を列挙し、`types` は
