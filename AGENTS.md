...
- 公開した識別子でも、名前やシグネチャの言い換えにしかならない doc コメントは書かない。
  同じ説明はパッケージの doc・型や関数の doc・docs/ のどれか 1 か所に置き、ほかで繰り返さない。
  「まだ〜は無い」「今のところ」のように今の状態を前提にした書き方は、コードが変わると
  黙って古くなるので避ける（典型的な言い回しは `diff-content` が止める）
