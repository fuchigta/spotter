...
- まだ push していないなら、検査を足すコミットより前だけを先に push してから残りを
  push してください（例: `git push origin <検査を足すコミットの親>:<ブランチ>`）。
  per-commit 粒度の検査は各コミットを個別に見るので、後のコミットで違反を直しても
  通りません。古いコミットのまま通したいなら、履歴を書き換えて各コミットを直すか
  免除トレーラを付けることになります
- 既に push して CI が落ちた場合は、PR を検査を足すコミットの前後で分け直すなど、
  範囲を区切り直してください
