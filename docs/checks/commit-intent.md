...
`allow` と `deny_diff` は**削除ファイルも見ます**（`docs:` と名乗りながらコードを削除しても
逃げられないように）。削除ファイルが違反に含まれる場合、パスの末尾に `（削除）` を付けて
表示で削除だとわかるようにします。一方 `require` は「変更ファイルの少なくとも 1 つ」を求める
ルールなので、**削除では満たしたことになりません**（テストを消したコミットで、消した
