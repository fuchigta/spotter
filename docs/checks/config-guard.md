...
全コミットを実行時の `.spotter.yml` で検査するため、途中で検査を足すコミットより前が
CI でだけ落ちることがある、という例外を持っています
（[ci-integration.md](../ci-integration.md)参照）。config-guard は、この「途中で緩めて
後で戻す」という迂回そのものを比較元・終点の差分から機械的に捕まえるための検査で、
...
どれにも無ければ起動しません。この「和」を取っているのは、config-guard を**追加する
コミット自身**が他の検査を緩めていても取りこぼさないためです。比較元にまだ config-guard
