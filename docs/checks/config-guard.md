...
**対象を絞らない全体免除（`skip <理由>`）は受け付けません。** config-guard は
[スコープ付き免除](../exemptions.md)（`skip[対象,...] 理由`）だけを認め、これは設定では
変えられません。対象を絞らない免除トレーラを書いても、検査は error にはならず違反を
残したまま「config-guard の免除には対象の指定（skip[対象]）が要ります」と案内します。

これは squashed 粒度の免除が「範囲内のどれか 1 コミットにあれば範囲全体に効く」ことに
由来する穴を塞ぐためです。全体免除を許すと、同じ push の中でコミット 1 が対象を絞らない
免除付きで検査 A を緩め、コミット 2 がトレーラ無しで検査 B を緩めても、コミット 1 の
免除がコミット 2 の緩和まで一緒に免除してしまいます。対象を絞った免除なら、コミット 1 の
免除は指定した対象（A）にしか効かないため、コミット 2 の緩和（B）は免除されずに残ります。

対象の単位は検査キー単位です。

| 対象 | 免除できる緩和 |
|---|---|
| `checks.<key>` | その検査キー配下の緩和すべて（`type` の変更や `exempt` を含む） |
| `types.<t>` | その type 配下の緩和すべて |
| `required_version` | `required_version` を下げた・消した緩和 |
| 設定ファイルのパス（例: `.spotter.yml`） | 個々の `checks.<key>` を突き合わせられない設定ファイル全体の緩和（YAML として解析できない・終点で削除された・未知のトップレベルキーの変更） |

指定できる対象の一覧は、比較元・終点それぞれの設定にある検査キー・type の和に
`required_version` と設定ファイルのパスを加えたものです（比較元にだけある、削除された
検査のキーも指定できます）。一覧に無い対象（書き間違い）を指定すると error になります。
