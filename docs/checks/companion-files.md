...
> 変更集合に含まれる `paths` 一致ファイルについて、その相方が**現在の作業ツリーに
> 存在しない**なら違反。

追加・変更を区別する必要が無く、既存のインターフェースだけで実装できます。相方の**中身**が
更新されているかまでは見ません（それは doc-sync の守備範囲で、こちらは存在だけを見ます）。

相方の実在確認は常に「現在の作業ツリー」を見ます。range モードで過去のコミットを検査する
場合、厳密にはそのコミット時点のツリーではありませんが、[doc-paths](doc-paths.md) /
[consistency](consistency.md) と同じ割り切りです。
