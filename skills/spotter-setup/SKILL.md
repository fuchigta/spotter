...
手順1〜6は互いに独立した調査なので、順不同で構いません。該当しないものは飛ばしてください。
...
8. **空振りテスト** — `spotter check --config <一時パス> --range "-10 HEAD"`
   のように、既存のコミット履歴に対して実際に走らせる（コミットが 10 件に満たなければ
   最初のコミットまで全部を見る）。既存履歴にどれだけ引っかかるかを見てから次に進みます
...
- `worktree` 粒度の検査（`doc-paths` / `consistency` / `doc-links`）は免除トレーラが
  無く、誤検知があると手直しするまで検査が通りません。空振りテスト（手順8）で
  誤検知が無いことを確かめてから入れます。裏を返せば、空振りテストさえ通っていれば
