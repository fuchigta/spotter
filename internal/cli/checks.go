		...
		Fields: []FieldInfo{
			{
				...
				Fields: []FieldInfo{
					...
					{Key: "exclude", Type: "array", ItemType: "string", Required: false, Description: "この pair だけを対象から外す doublestar パターンの一覧。トップレベルの exclude と併用でき、どちらかに一致すれば対象から外れる（他の pairs には影響しない）"},
				},
			},
		...
		Fields: []FieldInfo{
			{
				...
				Description:   "集合を抜き出す元。file と glob はどちらか一方が必須",
				RequiredOneOf: [][]string{{"file", "glob"}},
				Fields: []FieldInfo{
					{Key: "file", Type: "string", Required: false, Description: "対象ファイルのパス。glob とは併用不可"},
					{Key: "line", Type: "string", Required: false, Description: "マッチさせる行を絞る正規表現（省略時は全行）。file 専用（glob では起動時エラー）"},
					{Key: "until", Type: "string", Required: false, Description: "指定すると line にマッチした行から until にマッチする行まで（両端含む）を 1 ブロックとして対象にする（line とセットでのみ指定可）。file 専用（glob では起動時エラー）"},
					{Key: "extract", Type: "string", Required: false, Description: "行に当てる正規表現（キャプチャグループをちょうど 1 つ含む必要がある）。file を使う場合は必須、glob では起動時エラー"},
					{Key: "split", Type: "string", Required: false, Description: "キャプチャした文字列をさらに分割する区切り文字。file 専用（glob では起動時エラー）"},
					...
					{Key: "glob", Type: "string", Required: false, Description: "対象ファイルの doublestar パターン。一致したファイルパスの一覧をそのまま集合にする（ディレクトリ・.git 配下は除く）。file とは併用不可"},
					{Key: "base", Type: "string", Required: false, Description: "glob 専用。一致したパスからこの接頭辞ディレクトリを取り除いた相対パスを要素にする（配下に無いパスが一致したら実行時エラー）"},
					{Key: "exclude", Type: "array", ItemType: "string", Required: false, Description: "glob 専用。この doublestar パターンの一覧に一致するパスを集合から除く（base を取り除く前のルート相対パスに当てる）"},
				},
			},
		},
	},
